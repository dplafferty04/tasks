...
export function makeMath(addends: number[]): string {
    ...
    const joinedAddends = addends.join("+");
    
    return `${sum}=${joinedAddends}`;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    const result = firstNegativeIndex !== -1
        ? values.slice(0, firstNegativeIndex + 1).concat(totalSum, values.slice(firstNegativeIndex + 1))
        : values.concat(totalSum);
    return result;
}